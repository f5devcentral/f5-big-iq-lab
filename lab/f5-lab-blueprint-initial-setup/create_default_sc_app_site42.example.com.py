...
# Get the template
the_url = HOST + TEMPLATES_URL + "?$filter=name%20eq%20'Default-f5-HTTP-lb-template'"
print(the_url)
templates = session.get(HOST + TEMPLATES_URL + "?$filter=name%20eq%20'Default-f5-HTTP-lb-template'").json()
template_link = templates['items'][0]["selfLink"]

# Get the target device
the_url = HOST + DEVICES_URL  + "?$filter=managementAddress%20eq%20'10.1.1.7'"
device = session.get(HOST + DEVICES_URL  + "?$filter=managementAddress%20eq%20'10.1.1.7'").json()['items'][0] # Just pick the first one
        ...
        "ltm:virtual::b487671f29ba": [
            {
                ...
                "subcollectionResources": {
                    ...
                    "security-shared:virtuals:4c90b4185ffa": [
                        {
                            "parameters": {},
                            "parametersToRemove": []
                        }
                    ...
                    "members:5109c66dfbac": [
                        {
                            "parameters": {
                                "port": 80,
                                "nodeReference": {
                                    "link": "#/resources/ltm:node:9e76a6323321/10.1.20.113",
                                    "fullPath": "# 10.1.20.113"
                                }
                            },
                            "parametersToRemove": []
                        },
                        {
                            "parameters": {
                                "port": "80",
                                "nodeReference": {
                                    "link": "#/resources/ltm:node:9e76a6323321/10.1.20.114",
                                    "fullPath": "# 10.1.20.114"
                                }
                            },
                            "parametersToRemove": []
                        }
